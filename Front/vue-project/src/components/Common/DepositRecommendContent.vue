<template>
    <div class="table-container mt-7">
        <v-data-table v-if="props.recoDeposits && props.recoDeposits.length" :headers="headers" :items="props.recoDeposits" class="elevation-1">
            ...
            <template v-slot:item="{ item }">
                <tr>
                    ...
                    <td>{{ getInterestRate(item.deposit_options, 6) }}</td>
                    <td>{{ getInterestRate(item.deposit_options, 12) }}</td>
                    <td>{{ getInterestRate(item.deposit_options, 24) }}</td>
                    <td>{{ getInterestRate(item.deposit_options, 36) }}</td>
...
onMounted(() => {
  console.log('Mounted:', props.deposits)
})
