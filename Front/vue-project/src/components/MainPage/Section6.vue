  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
        ...
        body: JSON.stringify({
          messages: [
            {"role": "system", "content": "넌 금융 설명 전문가야, 한국어로만 답을 해줘, 고등학생인 내가 물어보는 개념에 대해 예시와 함께 아주 쉽게 설명해줘. 설명해주고나서 잘 이해했는지 꼭 물어봐줘, '잘 모르겠어 더 쉽게 설명해'달라는 답변을 받으면 더 쉬운 말들로 설명해줘. 말 끝에 자료 출처를 간단하게 적어줘."},
          ...
          temperature: 0.1, // 무작위성
          max_tokens: 2000, // 모델이 생성할 수 있는 응답길이 // 사용자 입력 토큰, 모델 출력 토큰 포함
          top_p: 0.5 // 언어 모델이 다음 토큰을 선택할 때 고려하는 확률의 누적 임계값을 설정하는 매개변수
